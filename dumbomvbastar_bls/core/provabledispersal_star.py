...
from crypto.threshsig.boldyreva import TBLSPrivateKey, TBLSPublicKey, serialize, deserialize1
...
def provabledispersalbroadcast(
    ...
    while True:
        ...
        if msg[0] == 'STORE':
            ...
            sig = SK1.sign(digest1, allow_pickle=True)
            # sig = serialize(sig)
        ...
        # receiving STORED message from pi
            ...
            try:
                ...
                assert PK1.verify_share(sigma, sender, digest, allow_pickle=True)
            ...
            if len(stored[roothash]) == SignThreshold and not LOCKSEND:
                ...
                try:
                    __Sigmas1 = dict(list(storedSigShares.items())[:SignThreshold])
                    Sigmas1 = PK1.combine_shares(__Sigmas1, allow_pickle=True)
                except Exception as e1:
                    if logger: logger.error(str(e1))
                    if logger: logger.error(traceback.format_exc())
                try:
                    digest = PK1.hash_message(str(('STORED', sid, roothash)))
                    assert PK1.verify_signature(Sigmas1, digest)
                except AssertionError as e1:
                    if logger: logger.error(str(e1))
                    if logger: logger.error(traceback.format_exc())
                except Exception as e2:
                    if logger: logger.error(str(e2))
                    if logger: logger.error(traceback.format_exc())
