...
from dumbomvbastar_bls.core.provabledispersal_star import provabledispersalbroadcast
from dumbomvbastar_bls.core.recast import recastsubprotocol
        ...
        def make_under_predicate():
            def vaba_predicate(vj):
                ...
                try:
                    ...
                    #     return False
                    digest = PK1.hash_message(str(('STORED', sid + 'PD' + str(id_j), roothash)))
                    assert PK1.verify_signature(Sigma, digest, allow_pickle=True)
                except Exception as e:
