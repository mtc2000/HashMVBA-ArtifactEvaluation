...
def reliablebroadcast(sid, pid, N, f, leader, basetime, rbc_time, rbc_time_interval, input, receive, send):
    """Reliable broadcast
    ...
    def decode_output(roothash):
        ...
        rbc_time[leader] = round(time.time() - rbc_time[leader], 2)  ####!!!!
        start_time, end_time = rbc_time_interval[leader]
        rbc_time_interval[leader] = (start_time, round(time.time() - basetime, 2))
        return m

    while True:  # main receive loop
        sender, msg = receive()

        if start_or_not is False:
            rbc_time[leader] = time.time()
            rbc_time_interval[leader] = (round(time.time() - basetime, 2), -1)
            start_or_not = True
