...
class DUMBOMVBA_STAR():
    def __init__(self, sid, pid, B, N, f,
        ...
        self.round_stop_normal_recv_events: Dict[int, Event] = defaultdict(Event)
        self.round_end_events: Dict[Any, Event] = defaultdict(Event)
        self.round_end_send_events: Dict[Any, Event] = defaultdict(Event)
        self.round_end_queue: Dict[int, Queue] = defaultdict(Queue)
    ...
    def round_thread_cleanup(self):
        for r in range(self.K + self.countpoint):
            self.round_end_events[r].wait()
            gevent.sleep(0)
            while True:
                try:
                    t: gevent.Greenlet = self.round_threads[r].get_nowait()
                    t.kill()
                except Empty:
                    break
    ...
    def _run(self):
        ...
        def _recv():
            ...
            while True:
                ...
                if msg == 'ROUNDEND':
                    self.round_end_queue[r].put(sender)
                    if self.round_end_queue[r].qsize() == self.N:
                        self.round_end_send_events[r].wait()
                        self.round_end_events[r].set()
                    continue
                if r == 'END':
                    self.round_end_queue['END'].put(sender)
                    if self.round_end_queue['END'].qsize() == self.N:
                        self.round_end_events['END'].set()
                    continue

                # actively dispose message if round r has outputted
                if self.round_stop_normal_recv_events[r].ready():
                    self.logger.warning(f'MESSAGE DROPPED: {_msg}')
                    del msg
                    continue
        ...
        self.send(-2, ('END', ''))
        self.send(self.pid, ('END', ''))
        self.round_end_events['END'].wait()
        gevent.sleep(5  + self.N / 40)
    ...
    def _run_round(self, r, tx_to_send, send, recv):
        ...
        #     round_input_queue.get,
        #     round_output_queue.put_nowait,
        #     recv, send,
        ...
        _t.join()
        self.round_stop_normal_recv_events[r].set()
        def send_round_end():
            self.send(-2, (r, 'ROUNDEND'))
            self.send(self.pid, (r, 'ROUNDEND'))
            # gevent.sleep(0.1  + self.N / 100)
            self.round_end_send_events[r].set()
        gevent.spawn(send_round_end)

        self.logger.info(f'round {r} stopping')
        self.round_end_events[r].wait()
        self.logger.info(f'round {r} stopped')
